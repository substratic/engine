...
Value physics_make_sphere_msc(VM *vm, int arg_count, Value *args) {
  ...
  return OBJECT_VAL(mesche_object_make_pointer(vm, sphere, true));
}

Value physics_sphere_intersect_msc(VM *vm, int arg_count, Value *args) {
...
Value physics_sphere_center_x_set_msc(VM *vm, int arg_count, Value *args) {
  SubstSphere *sphere = (SubstSphere *)AS_POINTER(args[0])->ptr;
  sphere->center_x = AS_NUMBER(args[1]);
  return args[1];
}

Value physics_sphere_center_y_set_msc(VM *vm, int arg_count, Value *args) {
  SubstSphere *sphere = (SubstSphere *)AS_POINTER(args[0])->ptr;
  sphere->center_y = AS_NUMBER(args[1]);
  return args[1];
}
